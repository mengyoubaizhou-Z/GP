...
def prepare_feat_proj_data_lists(
    features, extrinsics, near, far, num_samples
):
    # prepare features
    ...
    if v == 2:
        ...
        pose_curr_lists = [torch.cat((pose, pose.float().inverse()), dim=0),]
...
class DepthPredictorMultiView(nn.Module):
    ...
    def forward(
        ...
        feat_comb_lists, pose_curr_lists, disp_candi_curr = (
        ...
        if visualization_dump is not None:
            visualization_dump["depth_wo_refine"] = rearrange(
                1 / fullres_disps,
                "(v b) 1 h w -> b v h w",
                v=v,
            )
        outputs["depth"] = 1 / rearrange(fullres_disps, "(v b) 1 h w -> b v h w", v=v)
        ...
        return depths, densities, raw_gaussians, outputs
