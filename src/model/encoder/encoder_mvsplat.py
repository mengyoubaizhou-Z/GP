...
@dataclass
class EncoderMVSplatCfg:
    name: Literal["mvsplat_enc"]
    d_feature: int
...
class EncoderMVSplat(Encoder[EncoderMVSplatCfg]):
    ...
    def __init__(self, cfg: EncoderMVSplatCfg) -> None:
        super().__init__(cfg)
    ...
    def forward(
        ...
        depths, densities, raw_gaussians, mvs_outputs = self.depth_predictor(
            in_feats,
            context["extrinsics"],
            context["near"],
            context["far"],
